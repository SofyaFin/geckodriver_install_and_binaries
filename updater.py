...
def pypi_exists(version):
    url = f"https://pypi.org/pypi/geckodriver-py/{version}/json"
    response = requests.get(url)
    return response.status_code == 200


def download_binaries(channel):
    for p in channel["downloads"]["geckodriver"]:
        platform = p["platform"]
        url = p["url"]
        ...
        origin = os.path.join(path, "geckodriver")
        dest = os.path.join("geckodriver_py", f"geckodriver_{platform}")
